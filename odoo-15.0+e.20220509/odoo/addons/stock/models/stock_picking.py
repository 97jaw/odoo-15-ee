...
class Picking(models.Model):
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env['stock.move'].read_group(
            [('id', 'in', self.move_lines.ids), ('delay_alert_date', '!=', False)], ['delay_alert_date:max'],
            'picking_id')
        ...
        for picking in (self - picking_no_alert):
            picking.json_popover = json.dumps({
                ...
                'late_elements': [{
                    ...
                    picking.move_lines.filtered(lambda m: m.delay_alert_date).move_orig_ids._delay_alert_get_documents()
                ]
            })
    ...
    def _compute_date_deadline(self):
        for picking in self:
            if picking.move_type == 'direct':
                picking.date_deadline = min(picking.move_lines.filtered('date_deadline').mapped('date_deadline'),
                                            default=False)
            else:
                picking.date_deadline = max(picking.move_lines.filtered('date_deadline').mapped('date_deadline'),
                                            default=False)
    ...
    def onchange_partner_id(self):
        for picking in self:
            picking_id = isinstance(picking.id, int) and picking.id or getattr(picking, '_origin',
                                                                               False) and picking._origin.id
            if picking_id:
    ...
    def _onchange_locations(self):
        ...
        if self.picking_type_id.code == 'internal' and not is_immediate and from_wh and to_wh and from_wh != to_wh:
            return {'warning': {
                'title': _("Warning"),
                'message': _(
                    "You should not use a planned internal transfer to move some products between two warehouses. "
